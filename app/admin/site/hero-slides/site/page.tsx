...
export default function HeroSlidesSitePage() {
  ...
  const loadSlides = async () => {
    try {
      ...
      // Filter only for desktop/site display (where device_type is 'site' OR null for backward compatibility)
      const siteSlides = (response.data || []).filter((slide: any) => slide.device_type === 'site' || slide.device_type === null);
      setSlides(siteSlides);
    } catch (error: any) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-black text-[#163A2C]">Bannieres Hero - Site Web</h1>
          <p className="text-sm text-[#163A2C]/60 mt-1">Gérez les bannieres du carrousel principal pour le site web</p>
        </div>
        <button
      ...
      {loading ? (
        ...
        <div className="py-16 text-center bg-white rounded-2xl border border-dashed border-[#163A2C]/20">
          <p className="text-[#163A2C]/60 font-medium">Aucune bannière créée pour le site web</p>
          <button
            onClick={handleCreate}
