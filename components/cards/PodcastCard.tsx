...
import { Edit2, Trash2, Play, Clock, Mic2, Crown } from "lucide-react";
import Image from "next/image";
import { useState } from "react";
import type { Podcast, PodcastStatut } from "@/types/admin";

// Permet aux autres fichiers d'importer Podcast depuis ici sans le redéfinir
export type { Podcast };
...
const STATUS_STYLES: Record<PodcastStatut, { label: string; className: string }> = {
...
/** Statut affiché : `status` (EN, normalisé) sinon fallback sur `statut` (FR) */
...
export default function PodcastCard({ podcast, onEdit, onDelete, onPlay, onClick }: Props) {
    ...
    const status = STATUS_STYLES[resolveStatut(podcast)];
    const categoryColor =
        CATEGORY_COLORS[podcast.programme?.categorie.toLowerCase() ?? "default"] ??
        CATEGORY_COLORS.default;
    const durationLabel = podcast.duree_formatee ?? formatDuration(podcast.duree ?? undefined);
        ...
        >
            ...
            <div className="relative h-48 w-full overflow-hidden bg-gradient-to-br from-[#163A2C] to-[#0E241C]">
                ...
                <div className="absolute top-3 left-3">
                    <span
                        className={`inline-flex items-center rounded-full px-2.5 py-1 text-[10px] font-bold uppercase tracking-wider backdrop-blur-md ${status.className}`}
                    >
                        {status.label}
                    </span>
                </div>
                ...
                {podcast.is_premium && (
                    <div className="absolute top-3 right-3">
                        <span className="inline-flex items-center gap-1 rounded-full bg-[#F0A93E] px-2 py-1 text-[10px] font-black tracking-wider text-[#0E241C] backdrop-blur-md">
                ...
                {/* Duration Badge */}
                {(podcast.duree != null || podcast.duree_formatee) && (
            ...
            <div className="flex flex-1 flex-col bg-[#FAF7F0] p-4">
                ...
                <p className="mt-1.5 text-xs font-medium text-[#163A2C]/60">
                    {podcast.programme?.titre ?? "Sans programme"}
                </p>
                ...
                <div className="mt-3.5 flex items-center justify-between border-t border-[#163A2C]/10 pt-3">
                    <div className="flex items-center gap-2 text-xs font-medium text-[#163A2C]/60">
                        <span>👁️ {podcast.vues ?? 0}</span>
                        <span>💬 {podcast.commentaires_count ?? 0}</span>
                    </div>
                    <div className="flex gap-1 opacity-0 transition-opacity group-hover:opacity-100">
                        <button
                            onClick={(e) => { e.stopPropagation(); onEdit(podcast); }}
                            aria-label={`Modifier ${podcast.titre}`}
                        ...
                        <button
                            ...
                            aria-label={`Supprimer ${podcast.titre}`}
